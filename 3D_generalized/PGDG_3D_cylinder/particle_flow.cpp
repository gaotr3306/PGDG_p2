...
void Particle_base::Grid_acceleration(SOLVER *mpm, int ss)
{
  ...
  #if DIM ==2
  mij<< 1., pow(ps(0),2)/3., pow(ps(1),2)/3.;// pow(ps(0)*ps(1),2)/9, pow(ps(0),4)/5, pow(ps(1),4)/5; 
  #elif DIM ==3
  mij<< 1., pow(ps(0),2)/3., pow(ps(1),2)/3., pow(ps(2),2)/3.;// pow(ps(0)*ps(1),2)/9, pow(ps(0),4)/5, pow(ps(1),4)/5; 
  #endif //2026.2.18
  mij *= voli; 
  ...
  for (int j = 0; j < int(Celli.cell_id.size()); j++)
  {
    ...
    #endif
    {
      ...
      Vector<Real, UNum> Ui; 
      Ui(0) = rho_i;
      Ui.segment<DIM>(1) = rhov_i.head(DIM);//2026.2.18
      Ui(UNum-1) = E_i;
      ...
      if(b_mk == 0)
      {
        ...
        for(int s1=0;s1< FldNum; s1++)
        {
          ...
          U_sL(0) = rho_sL;
          U_sL.segment<DIM>(1) = rhov_sL.head(DIM);//2026.2.18
          U_sL(UNum-1) = E_sL;
          U_sR(0) = rho_sR;
          U_sR.segment<DIM>(1) = rhov_sR.head(DIM);//2026.2.18
          U_sR(UNum-1) = E_sR;
      ...
      else if(b_mk == 1)
      {
        Vector<Real, UNum> Uj; 
        Uj(0) = rho_i;
        Uj.segment<DIM>(1).setZero();//2026.2.18
        Uj(UNum-1) = E_i;
      ...
      else if(b_mk == -1 || b_mk == -2 || b_mk == -3) //inlet and outlet
      {
        ...
        //   {
        //     P_j = 100000; 
        //     E_j = mpm->EOS_rhoe(P_j, ss) + 0.5* v_j.head(DIM).squaredNorm()* rho_j;
        //   }
        // }
        // else if(b_mk == -3)
        // {
