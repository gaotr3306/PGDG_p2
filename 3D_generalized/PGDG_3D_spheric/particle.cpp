...
void Particle_base::Refresh_neighbor_info(MPM *mpm, int flag)
{
  ...
  current_level->Refresh_neighbor_info(mpm, this, pos, flag);	//2026.7.3
...
// Set_ptcl 20260618
//-----------------------------------------------------
void Particle_base::Set_ptcl(p_Particle cp, MPM *mpm)
{
  vol = cp->vol;
  phase= cp->phase;
  mass = cp->mass;
  ...
  coef0[0] = coef[0];//2026.1.22
  rho[0] = cp->rho[0] ;
  rhov[0].head(DIM) = cp->rhov[0].head(DIM) ;//2026.7.25
  E[0] = cp->E[0];
