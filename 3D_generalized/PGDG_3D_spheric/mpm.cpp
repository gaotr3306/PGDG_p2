...
void MPM::Merge_particle(communicator &world) 
{
  ...
  static affinity_partitioner ap;
	parallel_for( blocked_range<int>(0, int(particle_fluid.size())),[&](const blocked_range<int>& r){
		...
		// if (cp->phase == 1 && (cp->coord.j < box_l.j || cp->coord.j >box_r.j || cp->vol < 0.1* cp->volk )) // 20251008: important for stabilization
		// if (cp->phase == 1 && (cp->coord.j < box_l.j || cp->coord.j >box_r.j  || cp->mass < 1e-12))
		// {
		// 	cp->out = 1;
		// }
