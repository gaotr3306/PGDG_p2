...
void MPM::Initialize_case(Initialization &Ini, communicator &world)//2023.12.22
{
  ...
  p_sz = domain.j/300 * Rt ;//

  p_g = domain.j/300 * Rt ;//2026.1.12
  ...
  for( p_id.i= 0; p_id.i!= glbl_num_particle.i ; ++p_id.i)
    for( p_id.j= 0; p_id.j!= glbl_num_particle.j ; ++p_id.j)
      ...
      if ( f1 )
      {
        ...
        int ss = flu_particle->phase       = 1; //2026.7.8
        flu_particle->P       = 0.1; 
        ...
        flu_particle->c[0]         = sqrt(gamma[ss-1]*(flu_particle->P + Pp0[ss-1])/flu_particle->rho[0]);
        flu_particle->E[0]       = (flu_particle->P + gamma[ss-1]* Pp0[ss-1])/(gamma[ss-1]-1)  ; 
