...
void Particle_base::Set_split_ptcl(p_Particle cp, MPM *mpm)
{
  ...
  vol = cp->vol/pow(2,DIM);
  phase= cp->phase;
  mass = cp->mass/pow(2,DIM);//2026.9.21
  h = cp->h/2.0;
  level = cp->level; //2026.9.21

  coef[0] = cp->coef[0];//2026.1.22
  coef0[0] = coef[0];//2026.1.22
  rho[0] = cp->rho[0] + coef[0].row(0).rightCols(BaseNum)* Basis_f(dr).tail(BaseNum);
  rhov[0].head(DIM) = cp->rhov[0].head(DIM) + coef[0].middleRows<DIM>(1).rightCols(BaseNum)* Basis_f(dr).tail(BaseNum);//2026.7.24
  E[0] = cp->E[0]+ coef[0].row(UNum-1).rightCols(BaseNum)* Basis_f(dr).tail(BaseNum);
