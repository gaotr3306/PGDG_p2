...
void Particle_base::Grid_acceleration(SOLVER *mpm, int ss)
{
  ...
  #endif
  {
    ...
    Vector<Real, UNum> dU = LIM*(coef[ss].rightCols(BaseNum)* (Basis_f(dp).tail(BaseNum)));
...
/////////////////////////////////////////////////////////////////////////////////////
  ...
  for (int j = 0; j < int(mpm->cell_dg[idI.i][idI.j].cell_id.size()); j++)
  {
    ...
    bool b_mk = mpm->cell_dg[idI.i][idI.j].bound[j];
    ...
    for(int bi=0; bi<= BaseNum; bi++) SetZero(DFij[bi]); //2026.7.8
    ...
    for(int g1=0; g1<int(mpm->Gpf.size()); g1++)
    {
      ...
      Vector<Real, UNum> dUi = LIM*(coef[ss].rightCols(BaseNum)* Basis_f(dri).tail(BaseNum));
      ...
      if(b_mk == 0)
      {
        ...
        for(int s1=0;s1< FldNum; s1++)
        {
          ...
          Vector<Real, UNum> dUj = LIMj* neigh->coef[s1].rightCols(BaseNum)* (Basis_f(drj).tail(BaseNum));
          ...
          if(ss != s1)
          {
            ...
            E_j = rhoe_j + 0.5* rho_j* v_j.head(DIM).squaredNorm();//20260521  
              ...
              // if(isnan(ghost_i.norm())) cout<<"vnj="<<P_j<<endl;
          }
          ...
          G_Ui[ss] += Uij* normal.head(DIM).transpose()* Sj* gw *  alph_ij ;
      ...
      {
        Vector<Real, UNum> Uj; Uj<<rho_i, 0, 0, E_i;
        G_Ui[ss] += Uj* normal.head(DIM).transpose()* Sj* gw;
        for(int bi=0; bi<= BaseNum; bi++) 
        {
          Real N_bi = Basis_f(dri)(bi);
...
#ifdef _G_
    Vector<Real, DIM> Sg; SetZero(Sg); Sg<<0, GRAVITY;

    dcoef[ss].col(s).middleRows<DIM>(1) += coef[ss](0, s)* Sg* mij(s); 
    dcoef[ss](UNum-1, s) += coef[ss].col(s).middleRows<DIM>(1).dot(Sg)* mij(s); 
#endif
  ...
  // dcoef[ss].rightCols(BaseNum).fill(0);
...
//   int ss = phase -1;
//   // if(isnan(cell_i->G_Ui[ss].norm())) cout<<"vnj="<<endl;

//   Vector<Real, 3> dri = coord - cell_i->coord;
//   Vector<Real, UNum> Ui, dUdt ;
//   for(int t=0;t<UNum;t++)
//   {
//     Ui(t) = cell_i->coef[ss](t,0) + 1* cell_i->coef[ss].row(t).rightCols(BaseNum)* Basis_f(dri).tail(BaseNum);//cell_i->Lim_u(t)
//     dUdt(t) = cell_i->dcoef[ss](t,0) + 1 * cell_i->dcoef[ss].row(t).rightCols(BaseNum)* Basis_f(dri).tail(BaseNum);
//   }
//   dUdt += cell_i->G_Ui[ss]* tv.head(DIM); 
//   coef[0] = cell_i->coef[ss];//2026.1.22
//   Real divu = cell_i->G_v.trace();//2026.2.6

//   a.head(DIM) = (dUdt.segment<DIM>(1) - dUdt(0)*v.head(DIM))/rho[0];//2026.2.1
...
//     rhov[0].head(DIM) += dUdt.segment<DIM>(1)* glbl_timestep;
//     rho[0] += dUdt(0)* glbl_timestep; 
//     E[0] += dUdt(UNum-1)* glbl_timestep; 
...
//     rhov[0].head(DIM) = 0.5*( (rhov_0 + rhov[0]).head(DIM) + dUdt.segment<DIM>(1) * glbl_timestep ) ;
//     rho[0] = 0.5* (rho[0] + rho_0 + dUdt(0)* glbl_timestep);
//     E[0] = 0.5* (E[0] + E_0 + dUdt(UNum-1)* glbl_timestep);
...
//   // dv += Bound_pen(mpm)* glbl_timestep/2.;
//   // tv = v + dv;
...
void Particle_base::Update_state_onestep(p_Particle cell_i, int md, SOLVER *mpm )
{
  ...
  for(int t=0;t<UNum;t++)
  {
    Ui(t) = cell_i->coef[ss](t,0) + 1* cell_i->coef[ss].row(t).rightCols(BaseNum)* Basis_f(dri).tail(BaseNum);//
  }
  coef[0] = cell_i->coef[ss];//2026.1.22
  Real rho_i =  Ui(0);
...
{
  Real ca = mpm->glbl_max_v* 5. ; //2026.9.21
  Real P_b = rho[0]* ca* ca ; // mpm->
  SetZero(pst);
  SetZero(ph_sharp);
  Real v_ww=0;

  Real dx = h/CELL_RATIO* CUT_OFF;//2026.7.8
