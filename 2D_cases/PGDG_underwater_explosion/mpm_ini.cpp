...
void MPM::Initialize_case(Initialization &Ini, communicator &world)//2023.12.22
{
  ...
  domain.i = particle_box.i = 4. *Rt ;  //5.366
  domain.j = particle_box.j = 3 *Rt;
  domain.k = particle_box.k = 1;

  num_level =  1 ; 

  p_sz = domain.i/400 * Rt ;//

  p_g = domain.i/400 * Rt ;//2026.1.12
  ...
  Pb0 = 1.01325e5;//20260528
  ini_rho[0] = 1000. ;//
  Pp0[0] = 6e8;
  gamma[0] = 4.4;
  mu[0]      = 0 ; // 1e-3 
  c[0]      =  sqrt(gamma[0]* (Pp0[0] + 1.01325e5)/ini_rho[0]) ;// 

  ini_rho[1] =  1.225 ;// 1.225
  Pp0[1] = 0;
  gamma[1] = 1.4;
  mu[1]      = 0  ; // 1.8e-5 
  c[1]       =  sqrt(gamma[1]* (Pp0[1] + 1.e9)/ini_rho[1]) ; // 
...
//===================fluid===================================
  ...
  for( p_id.i= 0; p_id.i!= glbl_num_particle.i ; ++p_id.i)
    for( p_id.j= 0; p_id.j!= glbl_num_particle.j ; ++p_id.j)
      ...
      bool f1 = (  p_coord(1) < water_domain.j );
      bool f2 = (  pow(p_coord(0)-2, 2) + pow(p_coord(1) -1.2, 2) < 0.12*0.12 );
      ...
      if ( f1 )
      {
        flu_particle->rho_0        = ini_rho[0];
        flu_particle->mass        = powern(p_sz, DIM)* ini_rho[0];
        flu_particle->vol         = powern(p_sz, DIM);//
        flu_particle->phase       = 1; 
        flu_particle->P       = 1.01325e5; 

        flu_particle->rho[0]       = ini_rho[0];
        flu_particle->rho0[0]      = ini_rho[0];
        ...
        if(f2)
        {
          flu_particle->rho_0        = ini_rho[1];
          flu_particle->mass        = powern(p_sz, DIM)* ini_rho[1];
          flu_particle->vol         = powern(p_sz, DIM);//
          flu_particle->phase       = 2; 
          flu_particle->P       = 1e9;  //1e9
  
          flu_particle->rho[0]       = 1250; //1250
          flu_particle->rho0[0]      = ini_rho[1];
...
/////////////////////////////////////
  ...
  for (int j = 0; j < Ny ; ++j)
  {
    ...
    bmk=0;
    if(cell_j.j >=0 ) bmk = 0;
    else bmk = 2; //2026.7.13
    cell_dg[i][j].bound.push_back(bmk);
...
void MPM::Output_point(communicator &world, Real time1)
{
  ...
  Real Tmass1 = 0, Tmass2 = 0, Tmox=0, Tmoy=0,  Tener=0 ;
  for(int i=0; i!= int(particle_fluid.size()); ++i) //2026.9.21
  {
    p_Particle cp = particle_fluid[i];

    if(cp->phase ==1 )
      Tmass1 += cp->vol* cp->rho[0];
    else if(cp->phase ==2 )
      Tmass2 += cp->vol* cp->rho[0];

    Tener += cp->vol* cp->E[0];
  }

  Real gTmass1 = 0, gTmass2 = 0, gTmox=0, gTmoy=0,  gTener=0 ;
  for(int i=0; i!= int(Used_Ele_cen.size()); ++i)
  {
    p_Particle cp = Used_Ele_cen[i];
    Real voli = cell_dg[cp->idI.i][cp->idI.j].Vol; 

    gTmass1 += voli* cp->rho[0]* cp->alph[0];
    gTmass2 += voli* cp->rho[1]* cp->alph[1];

    for(int s1=0;s1< 2; s1++)
      gTener += voli* cp->E[s1]* cp->alph[s1];
    
  }

  Real p2g = Real(particle_fluid.size())/Real(Used_Ele_cen.size()) ;
  

if(Iden==1){
 ofstream out("./point_p.txt",ios::trunc);  
  out<<time1<<" "<<setprecision(6)<<Tmass1<<" "<<Tmass2<<" "<<Tener<<" "<<p2g<<" "<<"\n";
  out.close();

 ofstream out1("./point_g.txt",ios::trunc);  
  out1<<time1<<" "<<setprecision(6)<<gTmass1<<" "<<gTmass2<<" "<<gTener<<" "<<"\n";
  out1.close();


}else{
 ofstream out("./point_p.txt",ios::out|ios::ate|ios::app);  
  out<<time1<<" "<<setprecision(6)<<Tmass1<<" "<<Tmass2<<" "<<Tener<<" "<<p2g<<" "<<"\n";
  out.close();

  ofstream out1("./point_g.txt",ios::out|ios::ate|ios::app);  
  out1<<time1<<" "<<setprecision(6)<<gTmass1<<" "<<gTmass2<<" "<<gTener<<" "<<"\n";
  out1.close();
}
}
